...
//         editClassTypeInput.appendChild(option);
//       });
//       editClassTypeInput.disabled = false;
//     } else {
//       editClassTypeInput.disabled = true;
//     }
//     const timeOptions = [
//       "06:00 - 07:00",
//       "07:00 - 08:00",
...
//       "19:00 - 20:00",
//     ];
//     editScheduleTimeInput.innerHTML =
//       '<option value="">-- Chọn giờ --</option>';
//     timeOptions.forEach((time) => {
//       const option = document.createElement("option");
//       option.value = time;
//       option.textContent = time;
//       editScheduleTimeInput.appendChild(option);
//     });
//   }
...
//     const sortedSchedules = [...dataToRender].sort((a, b) => {
...
//                         <td>${escapeHtml(
//                           item.name || "N/A"
//                         )}</td> <td>${escapeHtml(item.email || "N/A")}</td>
...
//                               item.id
//                             }')" title="Sửa lịch này">Sửa</button>
//                             ${cancelButtonHTML}
//                             <button class="btn btn-secondary btn-sm" onclick="openNoteModal('${
//                               item.id
//                             }')" title="Thêm/Sửa ghi chú">Ghi chú</button>
//                         </td>
//                         <td>
//                             <button class="btn btn-danger btn-sm" onclick="confirmAdminDelete('${
//                               item.id
//                             }')" title="Xóa vĩnh viễn">Xóa</button>
//                         </td>
//                     </tr>
//                 `;
//         })
//         .join("");
//     } else {
//       scheduleTableBody.innerHTML = `<tr><td colspan="9" style="text-align:center;">Không có lịch tập nào.</td></tr>`;
//     }
...
//     } else {
//       prevLi.innerHTML = `<a href="#" data-page="${currentPage - 1}">«</a>`;
//     }
...
//     } else {
//       nextLi.innerHTML = `<a href="#" data-page="${currentPage + 1}">»</a>`;
//     }
...
//     setTimeout(() => {
//       editClassTypeInput.value = currentScheduleToEdit.type;
//       editScheduleTimeInput.value = currentScheduleToEdit.time;
//     }, 0);
//     editScheduleDateInput.value = currentScheduleToEdit.date;
//     editFullNameInput.value = currentScheduleToEdit.name;
//     editEmailInput.value = currentScheduleToEdit.email;
//     editScheduleNoteInput.value = currentScheduleToEdit.note || "";
//     editModal.style.display = "flex";
//   }
//   function handleEditFormSubmit(event) {
//     event.preventDefault();
//     if (!currentScheduleToEdit) return;
//     // Basic validation (can be enhanced)
//     if (
//       !editClassTypeInput.value ||
//       !editScheduleDateInput.value ||
//       !editScheduleTimeInput.value ||
//       !editFullNameInput.value.trim() ||
//       !editEmailInput.value.trim()
//     ) {
//       editModalError.textContent = "Vui lòng điền đầy đủ thông tin bắt buộc.";
//       return;
//     }
//     const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
//     if (!emailRegex.test(editEmailInput.value.trim())) {
//       editModalError.textContent = "Định dạng email không hợp lệ.";
//       return;
//     }
...
//     if (index === -1) {
//       editModalError.textContent = "Lỗi: Không tìm thấy lịch.";
//       return;
//     }
//     allSchedules[index].type = editClassTypeInput.value;
//     allSchedules[index].date = editScheduleDateInput.value;
//     allSchedules[index].time = editScheduleTimeInput.value;
//     allSchedules[index].name = editFullNameInput.value.trim();
//     allSchedules[index].email = editEmailInput.value.trim();
//     allSchedules[index].note = editScheduleNoteInput.value.trim();
//     saveSchedules();
//     renderTable();
//     hideAllModals();
...
//       if (
//         confirm(
//           `Admin: Chuyển lịch ngày ${allSchedules[index].date} của ${allSchedules[index].name} thành "Đã hủy"?`
//         )
//       ) {
//         allSchedules[index].status = "cancelled";
//         saveSchedules();
//         renderTable();
//       }
//     } else if (index !== -1) {
//       alert("Lịch này đã ở trạng thái hủy.");
//     } else {
//       alert("Không tìm thấy lịch.");
//     }
//   }
//   function openNoteModal(id) {
//     currentScheduleToAddNote = allSchedules.find((s) => s.id === id);
//     if (!currentScheduleToAddNote || !noteModal || !noteForm) return;
//     hideAllModals();
//     noteForm.reset();
//     noteScheduleIdInput.value = currentScheduleToAddNote.id;
//     scheduleNoteInput.value = currentScheduleToAddNote.note || "";
//     noteModal.style.display = "flex";
//   }
//   function handleNoteFormSubmit(event) {
//     event.preventDefault();
//     if (!currentScheduleToAddNote) return;
//     const index = allSchedules.findIndex(
//       (s) => s.id === currentScheduleToAddNote.id
//     );
//     if (index !== -1) {
//       allSchedules[index].note = scheduleNoteInput.value.trim();
//       saveSchedules();
//       renderTable();
//       hideAllModals();
//     } else {
...
//     }
//     if (
//       window.confirm(`XÓA VĨNH VIỄN lịch ${s.type} - ${s.date} của ${s.name}?`)
//     ) {
...
//       saveSchedules();
//       currentPage = 1;
//       renderTable(); /* updateStats(); if applicable */
//     } else {
//       alert("Lỗi: Không tìm thấy lịch.");
//     }
//   }
...
//       if (event.target === editModal || event.target === noteModal) {
//         hideAllModals();
//       }
//     });
//   });
